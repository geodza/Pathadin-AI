...
DATA = Path(os.environ.get('PATHADINAI_DATA',str(BASE/'data')))
...
async def local_only(request: Request, call_next):
    ...
    if request.method not in {'GET','HEAD','OPTIONS'} and not secrets.compare_digest(request.headers.get('x-pathadinai-token',''),TOKEN):
...
async def browse():
    ...
    def choose():
        ...
        try:
            ...
            result = subprocess.run([sys.executable,'-m','pathadinai.picker'],cwd=BASE,capture_output=True,timeout=180,
